              ...
              {data.transactions.map((txn) => (
                <tr key={txn.id} className="border-b border-border hover:bg-muted transition-colors">
                  <td className="px-6 py-4">
                    ...
                    <span className="text-xs capitalize px-2 py-0.5 mt-1 inline-block rounded bg-muted text-foreground">
                      {txn.type}
                    </span>
                  </td>
                  <td className="px-6 py-4 font-medium text-foreground">{txn.userName}</td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      {txn.status === 'failed' && (
                        <Button variant="outline" size="sm" onClick={() => handleRetry(txn.id)} className="bg-muted border-border">
                          <RotateCcw className="w-3 h-3 mr-1" /> Retry
                        </Button>
                      )}
                      {txn.status === 'success' && (
                        <Button variant="outline" size="sm" onClick={() => handleRefund(txn.id)} className="bg-muted border-border hover:text-red-500">
